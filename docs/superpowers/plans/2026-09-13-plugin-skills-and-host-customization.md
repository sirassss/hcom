...
- [x] **Step 4: Verify package artifact using Task 1's self-contained test.** (resolved by Task 6 Step 3, 2026-09-14: overlay selection confirmed at runtime — Codex read `hooks/hooks-codex.json`; local-marketplace install exercised the same selection path.) Read-only CLI schema validation plus scratch install/import must prove overlay selection; record CLI version. Official schema supports overlay paths, but import behavior is an acceptance measurement, not a claim based only on JSON validity.
