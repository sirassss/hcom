...
- [x] Extend Message with thread: Option<String> and delivery_known: bool. Keep delivered
...
- [x] Update every Message literal, using rg to find them rather than assuming two fixtures.
  Tests for broadcasts must set scope = Broadcast explicitly; clearing recipients alone
  leaves a Mentions fixture inconsistent.
- [x] Add a shared roster lookup in DataState, used by tag_of and resolve_display_name.
...
- [x] For remote agents, raw storage identity is base:device and display/action identity may
...
- [x] Test thread present/missing/null/non-string; delivery missing/null/empty/populated;
...
- [x] Run just test tui::db and just test tui::model, plus focused identity tests in state.rs.
  Run cargo check --locked before committing the task.

## Task 2: Shared tier, filter and collection

- [x] Declare pub mod filter in src/tui/mod.rs. Add FeedItem<'a> with borrowed Msg/Ev,
...
- [x] Add MsgFilter with tag/thread/to/from: Option<String>, text: String and
...
- [x] Add to_query() excluding roster selection, has_query(), has_tokens(), and is_empty().
  Round-trip only query fields; do not assert parse(to_query()) preserves agents.
- [x] Implement matches() and passes() according to spec §3. Conditions AND together;
...
- [x] to:X uses recorded delivered recipients when delivery_known is true, even when empty.
...
- [x] The roster condition matches sender or explicit recipients through identity lookup.
...
- [x] Free text uses Unicode lowercase substring matching against the original fields
...
- [x] Add collect_items(data, tier, filter) returning all matching borrowed items in
...
- [x] Tier admission matrix for messages, Tool and every Activity variant; tier cycle,
  valid/invalid string parsing and Compact default.
- [x] Parser unknown/empty/duplicate tokens, whitespace, empty input and query round-trip.
- [x] Broadcast with recorded bigboss matches to:bigboss but fails to:*.
...
- [x] from/tag/thread/to individually and together; case variants; roster conditions;
  local versus tagged remote identities, including a same-base collision.
- [x] Literal eliver, deliver:bono, 2026-09-10, Đồng/đồng, resolved event display name,
...
- [x] Shared collection order for equal timestamps and out-of-order IDs; filter before cap;
  counts independent of cap and collapsed lines. Default Compact count is messages only.
- [x] Run just test tui::filter and focused state tests, then cargo check --locked.
...
- [x] Add UiState.msg_tier and msg_filter; migrate both App construction sites. Delete
...
- [x] Migrate ui.selected readers throughout src/tui, especially actions.rs pruning/removal,
...
- [x] In Navigate mode, v cycles the tier, resets scroll and triggers inline replay.
...
- [x] Open Search with msg_filter.to_query(), placing the cursor at a valid end position.
...
- [x] Outside overlays, Esc clears text first, then the four structured fields, then agents,
...
- [x] Update App::active_search_query (in app.rs, not actions.rs) to return committed
  msg_filter.text only when nonempty. All highlighting uses this free-text value.
- [x] Preserve filter and tier across prepare_inline_viewport/prepare_vertical_viewport.
...
- [x] Update actual Ejector APIs: begin_replay and eject_new (there is no replay_all).
...
- [x] Clone admitted items into the existing owned replay queue. FeedItem stays borrowed;
...
- [x] Keep a pending-replay state even with zero items so the separator, empty message and
...
- [x] Fix incremental scan progress: capture old message/event watermarks before iterating;
...
- [x] Replace the vertical three-way dispatch with command-result override or the shared
...
- [x] Remove render_agent_feed, render_all_messages, render_events_timeline and local
...
- [x] Migrate render/mod.rs consumers now, including render_header, render_messages_separator,
...
- [x] Remove update_search and every caller; DataState.search_results and all initializers;
...
- [x] Exercise actual replay collection/emission helpers, not just passes() again.
...
- [x] All filter mutations (v, Enter/space, a, each Esc stage, Search commit and pruning)
...
- [x] Check Search prefill, preserved roster selection, full three-press Esc cascade,
  unchanged tier, empty commit, literal punctuation, and viewport switching.
- [x] Zero-match replay still emits filter context, empty state and live marker exactly
  once. Changing filters during a multi-chunk replay cannot emit stale queued items.
- [x] Replay followed by new live rows has no duplicate IDs; filtered rows advance progress.
  A time-ordered vector with IDs [12, 11] emits both when the old watermark is 10.
- [x] Seed a temporary DB with an old matching message outside the recent limit: it is
...
- [x] Preserve existing compose/action tests, especially
...
- [x] Reuse format_message, push_msg_header, event_line and shared run/line helpers.
  Do not fork formatting implementations between vertical and inline.
- [x] Keep named recipients as individual spans so each retains its own highlight and ✓.
...
- [x] Reserve dim time plus two trailing columns before clipping the header. Use Unicode
...
- [x] Wrap body to the available width with two-space hanging indent. Maintain one blank
...
- [x] Make build_waterlines accessible to shared formatting and key it with the same
...
- [x] Tool events render a single line in Normal and Verbose. Take only the first detail
...
- [x] Group consecutive admitted lifecycle events by canonical owner and absolute minute
...
- [x] In live inline output group only within the current emission batch and emit promptly.
...
- [x] Keep substring matching on original detail/sub-lines even when output is shortened.
...
- [x] Assert actual styled line output for a reply with four recipients (two names +2),
  individual ✓ spans, long tagged remote names, intent and right-aligned time.
- [x] Check wrapped Vietnamese/CJK/emoji body, long unbroken words and narrow widths with
  a ratatui TestBackend or pure line builder; all rendered rows fit their available width.
- [x] Check system message, empty/multiline tool detail and file path shortening.
- [x] Check lifecycle runs of 1/2/3, interruptions, different dates with the same HH:MM,
  and a run crossing a replay line-budget boundary.
- [x] Check inline and vertical shared formatting plus the documented live-batch exception.
- [x] Run just test tui before committing.

## Task 5: Coordinator config and B shortcut

- [x] Add bigboss to HcomConfig, its Default, get_field(), set_field(), normalization/
...
- [x] In src/config.rs add TOML_KEY_MAP entry ("bigboss", "preferences.bigboss") and
  FIELD_TO_ENV entry ("bigboss", "HCOM_BIGBOSS").
- [x] In src/commands/config.rs update CONFIG_KEYS with HCOM_BIGBOSS,
...
- [x] Load HcomConfig::load(None) once in production App startup; use its bigboss value
...
- [x] B in Navigate removes to: when it identifies the configured coordinator; otherwise
  replaces it with that coordinator. Use shared identity/case rules, preserve other fields,
  reset scroll and trigger replay.
- [x] Test get_field/set_field, defaults/empty/invalid values, TOML load/save/reload, environment
...
- [x] Test B twice, replacing another to:, case/alias variants and preserving text/agents/
  tag/thread/from. Test B is literal in Compose and Search.
- [x] Run focused config and CLI config tests, just test tui, then just test before commit.
...
- [x] Render tag/thread/to/from, sorted agent selections and quoted free text as chips in
...
- [x] Reserve count space before clipping chips; avoid pushing it off the right edge.
...
- [x] Footer says v tier:compact/normal/verbose. Update existing help entries in place,
  retaining other navigation/action controls and the scrollable help layout.
- [x] Explain '/' recent-window scope and grammar, Search Enter/cancel, B coordinator
...
- [x] Verify header/footer/help output at normal and narrow widths, filtered/unfiltered/
  empty windows and presentation cap. Check count tests use messages-only total in Compact.
- [x] Run focused render tests and just test tui before committing.
